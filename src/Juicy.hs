...
loadWithMetadata :: (FilePath -> IO (Either String (DynamicImage,Metadatas))) -> FilePath -> IO (Maybe (Picture,Metadatas))
loadWithMetadata reader fp = do
    eImg <- reader fp
    return $ either (const Nothing) (\(x,y) -> fmap (\x -> (x,y)) (fromDynamicImage x)) eImg
