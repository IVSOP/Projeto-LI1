...
É constituído por duas funções principais: 'constroiMapa' e 'desconstroiMapa'. 

Permitem convertar mapas entre os formatos:

* ['Peca','Coordenadas'] (sem indicação de peças 'Vazio').

* ['Peca'] (com indicação de peças 'Vazio').
...
constroiLinha :: [(Peca,Coordenadas)] -- ^ Mapa com apenas as peças da linha (através de 'getLinha').
 ...
 -> Int -- ^ Constante com o valor máximo x do mapa original (obtido de 'xMax'). 
 -> [Peca]
...
-- ** Função 'constroiMapaAux'

{- | Agrupa várias linhas construídas numa lista global. Para isso, chama recursivamente a função 'constroiLinha', aumentando em cada passagem o índice da linha construída.

Supõe-se que a lista já foi ordenada.

=== Propriedades:

prop> Em cada passagem, a linha de menor índice da lista é removida do mapa inicial ( através de 'getLinha') para tornar mais eficientes as passagens seguintes. 
Assim, quando 'getLinha' procura os elementos da linha seguinte do mapa, já não tem que passar pelos elementos da linha anterior.

prop> Quando o valor do índice de linha (valor y) , guardado no acumulador, atinge o valor máximo y do mapa (obtido por 'yMax'), a função percebe que chegou à última linha e fecha a lista.
...
Como a função percorre a lista da cima para baixo, para o funcionamento correto da função é necessário introduzir 0 neste parâmetro.
...
constroiMapaAux :: [(Peca, Coordenadas)] -- ^ Mapa original, mas ordenado (através de 'ordena').
 -> Int -- ^ Acumulador que guarda o valor y da linha em que a função está a trabalhar (0 no início, obrigatoriamente).
 -> Int -- ^ Constante com o valor máximo y da lista (obtida de 'yMax'). 
 -> Int -- ^ Constante com o valor máximo x da lista (obtida de 'xMax'). É necessário para a função 'constroiLinha'.
 -> Mapa
...
* Todos os elementos da linha 3 são colocados na primeira lista, e o restantes na segunda lista.

== Funções importadas

Para além das funções auxiliares referidas, também foram importadas da Tarefa 1 as funções:

* 'yMax' -  Devolve o y maximo do mapa.

* 'Xmax' -  Devolve o x maximo do mapa.
...
getLinha [] _ = ([],[])
getLinha l@(h@(_,(_,y)):t) yLinha = span ((==yLinha).snd.snd) l 

-- * Função desconstroiMapa

{- | Dada uma mapa de peças de todos os tipos, converte-o numa listagem das peças e respetivas coordenadas, excluindo as peças do tipo 'Vazio'.
...
-- ** Função 'desconstroiLinha'

{- | Dada uma linha de peças de todos os tipos, converte-a numa listagem das peças e respetivas coordenadas, excluindo as peças do tipo 'Vazio'.
...
* 0 é o índice de coluna da peça que a função irá converter primeiro. 
Como a função percorre a lista da esquerda para a direita, para o funcionamento correto da função é necessário introduzir 0 neste parâmetro.
...
-}
desconstroiMapaAux :: Mapa -- ^ Mapa original.
...
yMax2 :: [[Peca]] -> Int
yMax2 l = (length l) - 1  -- a linha 0 também existe

{- | Dado o índice de linha pretendido de uma mapa, devolve uma par com uma lista das peças da linha pretendida e uma lista das restantes peças, para maior eficiência de 'desconstroiMapa'.

 === Propriedades:

* Como esta função é usada em conjunto com 'desconstroiLinha', o índice da linha pretendida será sempre correspondente ao da primeira linha da tabela fornecida a esta função.
...
getLinha2 :: [[Peca]] -- ^ Mapa cuja primeira linha contem as peças que pretendemos separar.
    -> Int -- ^ Índice da linha que pretendemos.
    -> ([Peca],[[Peca]])
